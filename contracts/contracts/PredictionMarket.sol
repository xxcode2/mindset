 ...
 * @notice Non-custodial parimutuel binary (YES/NO) prediction markets.
 *
 * Mechanics:
 *  1. Anyone can create a market: question + closeTime + resolver address.
 *  2. Users place YES or NO bets in ETH while market is open (block.timestamp < closeTime).
 *  3. After closeTime, the resolver calls resolve(YES) or resolve(NO).
 *  4. Winners call claim() and receive a share of the entire pool proportional to their bet:
 *        payout = (userBet * totalPool) / winningPool
 *     A 1% protocol fee is taken from the losing pool and sent to feeRecipient.
 *  5. Safety net: if the resolver does not resolve within RESOLUTION_GRACE_PERIOD after
 *     closeTime, anyone can call markInvalid(); all bettors then call refund() to get
 *     their stake back.
 *
 * Trust model:
 *  - No owner, no admin, no upgrade path.
 *  - feeRecipient is set in the constructor and immutable forever.
 *  - The deployer cannot move user funds. Only contract logic does.
 *  - Each market has its own resolver chosen by the creator. Pick wisely.
 */
contract PredictionMarket {
    ...
    /// @notice user => market IDs they bet on
    mapping(address => uint256[]) private _userBets;
    /// @notice marketId => user => has been recorded in _userBets (prevents duplicates)
    ...
    function createMarket(
        ...
        emit MarketCreated(marketId, msg.sender, resolver, question, closeTime);
    }

    function bet(uint256 marketId, bool yes) external payable {
        ...
        emit BetPlaced(marketId, msg.sender, yes, msg.value, m.yesPool, m.noPool);
    }
    ...
    function resolve(uint256 marketId, bool yesWon) external {
        ...
        // Take fee from losing pool only (winners' stakes are returned + share of losers' pool minus fee)
    ...
    /// @notice Preview payout if `outcome` were the result and user has the given bet.
